...
section .data                   ; Data segment (initialized data)
    ...
    add_book_query db "Enter the Book Title You Want to Add (20 Characters): "
    ...
    restock_book_query_quant db "Enter the quantity (Max - 9): "
...
read_file: ; open and read book txt
    ...
    ; Store file descriptor as a dword
    mov dword [fd_in], eax
...
initialize_book_array: ; read from text file and places saved books into array
    ...
    ; If fileBuffer is empty (empty file or file doesn't exist)
    ...
    .parse_loop:
        ...
        call get_book_at_index ; edi points to index in book array
...
restock_book_menu:
    ...
    ; Book found, store index in ecx for restock_book
    movzx ecx, al
    ...
    .book_quant_loop:
        ...
        ; Check if Restocking Said Amount Exceeds The Limit of 10
        mov dl, byte [books_stock_borrowed + ecx]
        add al, byte [books_stock_available + ecx]
        add dl, al
        sub dl, '0'
        cmp dl, 10
        jge .book_quant_loop
    ...
    ; Show success message
    PRINT_STR book_restocked_msg, book_restocked_msg_len
    jmp .done
